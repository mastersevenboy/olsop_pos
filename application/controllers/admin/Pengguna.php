...
class Pengguna extends CI_Controller{
	...
	if($this->session->userdata('akses')=='1'){
		...
		$this->load->view('admin1/v_pengguna',$data);
	}else{
	...
	function Costumer(){
	if($this->session->userdata('akses')=='1'){
		$data['data']=$this->m_pengguna->get_coustumer();
		$this->load->view('admin1/v_costumer',$data);
	}else{
        echo "Halaman tidak ditemukan";
    }
	}
	...
	if($this->session->userdata('akses')=='1'){
		...
		$this->m_pengguna->hapus_costumer($kode);
		redirect('admin/pengguna/Costumer');
	}else{
        echo "Halaman tidak ditemukan";
    }
	}
	...
	if($this->session->userdata('akses')=='1'){
		...
		if ($password2 <> $password) {
			...
			$this->m_pengguna->simpan_pengguna($nama,$username,$password,$level,$status);
