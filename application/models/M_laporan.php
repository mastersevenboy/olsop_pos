...
class M_laporan extends CI_Model{
	...
	function get_minggu_jual(){
		$hsl=$this->db->query("SELECT DISTINCT YEARWEEK(jual_tanggal) AS minggu FROM tbl_jual");
		return $hsl;
	}
	...
	function get_jual_perminggu($minggu){
		$hsl=$this->db->query("SELECT jual_nofak,YEARWEEK(jual_tanggal) AS minggu,DATE_FORMAT(jual_tanggal,'%M %Y') AS bulan,DATE_FORMAT(jual_tanggal,'%d %M %Y') AS jual_tanggal,d_jual_barang_id,d_jual_barang_nama,d_jual_barang_satuan,d_jual_barang_harpok,d_jual_barang_harjul,d_jual_qty,d_jual_diskon,d_jual_total FROM tbl_jual JOIN tbl_detail_jual ON jual_nofak=d_jual_nofak WHERE YEARWEEK(jual_tanggal,'%M %Y')='$minggu' ORDER BY jual_nofak DESC");
		return $hsl;
	}
	function get_total_jual_perminggu($minggu){
		$hsl=$this->db->query("SELECT jual_nofak,YEARWEEK(jual_tanggal) AS minggu,DATE_FORMAT(jual_tanggal, '%M %Y') AS bulan,DATE_FORMAT(jual_tanggal,'%d %M %Y') AS jual_tanggal,d_jual_barang_id,d_jual_barang_nama,d_jual_barang_satuan,d_jual_barang_harpok,d_jual_barang_harjul,d_jual_qty,d_jual_diskon,SUM(d_jual_total) as total FROM tbl_jual JOIN tbl_detail_jual ON jual_nofak=d_jual_nofak WHERE YEARWEEK(jual_tanggal,'%M %Y')='$minggu' ORDER BY jual_nofak DESC");
		return $hsl;
	}
